...
10. **Partir o provisionamento de tenant em dois atos.** Decidido em 2026-08-01, durante `SEC-01A`. `provisionar_tenant` grava hoje em `tenant`, `tenant_modulo` e `audit_log` **e** nas tabelas de negócio do tenant (`utils.usuario`, `utils.grupo`, `protocolos.tipo_manifestante`), tudo sob o papel municipal. Isso contradiz o ADR-016 §2.3, que nega DML de entitlement a `aprimora_app` — mas mover o bloco inteiro para `aprimora_platform` seria pior: daria ao papel de plataforma DML nas tabelas de negócio dos tenants, exatamente o que o §2.3 lhe nega. A saída é **partir**, não afrouxar: criar a linha em `tenant` e a contratação inicial é ato de **plataforma**; povoar o tenant recém-criado é ato **municipal**, num segundo passo já no contexto daquele tenant. Efeito colateral desejado: o provisionamento ganha uma fronteira explícita onde hoje há um bloco monolítico que só funciona porque um papel pode tudo. Enquanto isso não acontece, `aprimora_app` mantém `INSERT` em `tenant`/`tenant_modulo`/`audit_log` e `UPDATE` em `tenant`, com a razão registrada em `_REVOGACOES` na migration `0076`.

**O que essa exceção deixa aberto, e que a revisão de segurança do `SEC-01A` apurou:** a fronteira de entitlement está fechada no HTTP e **aberta no SQL**. `aprimora_app` tem `INSERT` em `aprimora_py.tenant_modulo` (confirmado no banco: `tenant_modulo | aprimora_app | INSERT,SELECT`) e essa tabela **não tem RLS** — é tabela de plataforma, por decisão registrada. Logo, qualquer injeção ou defeito de service no runtime municipal insere uma linha de contratação para **qualquer** tenant, auto-contratando módulo que ninguém comprou. Alterar ou apagar não dá (o `REVOKE` de `UPDATE`/`DELETE` pegou); criar, dá. Agrava que `POST /admin/tenants` é a única das 8 rotas que ainda roda em `ged_user` (`SUPERUSER`/`BYPASSRLS`, F-12), então um defeito ali executa sem isolamento nenhum. Fechar esse `INSERT` é consequência direta de partir o provisionamento — é o mesmo item, e é por isso que ele não pode ficar para depois do `RBAC-01`.
