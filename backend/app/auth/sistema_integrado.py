...
5. **Tenant do host vs. tenant da chave.** O `TenantMiddleware` já resolveu
   ...
   gravado em `request.state.tenant_id` para que qualquer código downstream
   que leia o `state` (inclusive `get_db`, se vier DEPOIS desta dependência
   na cadeia) veja o tenant certo.
