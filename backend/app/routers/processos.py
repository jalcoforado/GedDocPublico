...
from ..schemas.processo import EncaminhamentoOut as _EncaminhamentoOut
...
async def create_endpoint(
    ...
    try:
        processo = await abrir_processo(db, payload, usuario_id=current.id)
    except AberturaError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    detail = await get_processo_detail(db, processo.id)
    if detail is None:
...
async def encaminhar_endpoint(
    ...
    try:
        await encaminhar(db, processo_id, payload, usuario_id=current.id)
...
async def receber_endpoint(
    ...
    try:
        await receber(db, processo_id, usuario_id=current.id)
    except AcaoError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    detail = await get_processo_detail(db, processo_id)
    if detail is None:
...
async def comprovante_pdf_endpoint(
    ...
    enc, uo, ud, prio = row
    detail = await get_processo_detail(db, enc.id_processo)
    if detail is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Processo não encontrado")
...
async def cancelar_encaminhamento_endpoint(
    ...
    try:
        enc = await cancelar_encaminhamento(
            db, encaminhamento_id, payload, usuario_id=current.id
        )
    except AcaoError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    detail = await get_processo_detail(db, enc.id_processo)
