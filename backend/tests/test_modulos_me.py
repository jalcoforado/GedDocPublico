...
`app.cli.seed_bootstrap` — não cria tenant novo. O backfill da migration 0073
só alcança tenants que já existiam quando ela rodou (ver
`test_modulos_migration.py::test_backfill_contratou_cinco_no_tenant_default`),
e o `sobral` é o único garantido nessa condição; por isso ele é a âncora
certa aqui, não um `two_tenants` recém-criado (que nasceria sem contratação
nenhuma).
...
async def test_me_devolve_apenas_contratados(admin_engine, http_client):
    ...
    contratados pelo backfill da migration 0073)."""
